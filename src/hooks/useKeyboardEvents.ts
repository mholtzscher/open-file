...
export function useKeyboardEvents(
  ...
  const handleNormalMode = useCallback((key: string) => {
    ...
    if (keySequenceRef.current.length === 0 || 
        (keySequenceRef.current.length === 1 && keySequenceRef.current[0] !== 'g' && 
         keySequenceRef.current[0] !== 'd' && keySequenceRef.current[0] !== 'y')) {
  ...
  const handleInsertMode = useCallback((key: string, char?: string) => {
    switch (key) {
      case 'escape':
  ...
  const handleSearchMode = useCallback((key: string, char?: string) => {
    switch (key) {
      case 'escape':
