...
export class BufferState {
  ...
  moveCursorDown(pageSize: number = 10): void {
    this.selection.cursorIndex = Math.min(this.entries.length - 1, this.selection.cursorIndex + 1);
  ...
  handleKeyPress(key: string): {
    handled: boolean;
    sequence: string[];
    action?: 'moveToTop' | 'moveToBottom' | 'delete' | 'copy' | 'help';
  } {
    ...
    if (
      this.keySequence.length === 1 &&
      (this.keySequence[0] === 'G' || this.keySequence[0] === 'shift+g')
    ) {
